...
for condition in model_part.ConditionIterators():
    if isinstance(condition, robust_outflow_condition.RobustOutflowCondition):
        ...
        for element in model_part.ElementIterators():
            if node1 in element.geometry.nodes and \
               node2 in element.geometry.nodes:
                [node3] = list(
                    set(element.geometry.nodes) - set([node1, node2]))
                print(node3.coordinates)
                condition.SetNormal(node3)
