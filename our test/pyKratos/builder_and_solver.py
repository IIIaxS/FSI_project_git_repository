# Authors: Wei He (wei.he@tum.com), Khalid Malik (khalid.malik@tum.com), Dennis Kasper (dennis.kasper@tum.com) (Implementations and manipulations are commented by "Group begin" and "Group end")
...
class BuilderAndSolver:
    ...
    def ComputeReactions(self, A, dx, b, dragLift):
        A, dx, b = self.Build(A, dx, b)
        print('ComputeReactions has been called')
        dragLift[0] = 0.0
        dragLift[1] = 0.0
        node_count = 0
        moment = 0.0
        for node in self.model_part.NodeIterators():
            if(node.GetSolutionStepValue(IS_STRUCTURE,0)):
                ...
                # the origin is in the square center at global x = 0 and global y = 0 (square l x b = 1.0 x 1.0)
                # get the lever arm in x and y direction, note that the origin of the square is at global (0, 0) and no relative lever arm needs to be considered here
                lx = node.coordinates[0]
                ly = node.coordinates[1]
                moment += b[node.EquationId(VELOCITY_Y)]*lx - b[node.EquationId(VELOCITY_X)]*ly
                print('dragLift[0] = ', dragLift[0], 'dragLift[1] = ', dragLift[1], 'moment = ', moment)
        return moment
	# Group end ===========================================