...
nameResultFile = "resultFileRes01.dat"

# clean up the result file
open(nameResultFile, "w").close()

# file header
file1 = open(nameResultFile, "a")
file1.write("dt" + "\t")
file1.write("M" + "\t")
file1.write("K" + "\t")
file1.write("B" + "\t")
...
f = 0.0
#for resonance try
f = Amp * sin(2* math.pi * freq * currentTime)
force.append(f)
...
#plt.ylim(-2.0, 2.0)
plt.title('$dt=%s, M=%s, K=%s, B=%s, u_0=%s, v_0=%s, a_0=%s, Amp=%s, f=%s $\n $ f(t) = Amp*cos(2*pi*f*t) $' % (dt, round(M,1), round(K,1), round(B,1), round(u0,1), round(v0,1) , round(a0,1), round(Amp,1), round(freq,3)))
